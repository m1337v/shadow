...
//     return result;
// }
static kern_return_t replaced_vm_region_recurse_64(
...
{
    if (isCallerTweak()) {
        return original_vm_region_recurse_64(target_task, address, size, nesting_depth, info, infoCnt);
    }

    // Block apps from scanning memory regions (common in jailbreak detection)
    return KERN_FAILURE;
}
